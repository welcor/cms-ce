...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan" version="1.0">
    ...
    <xsl:variable name="runtype">
      <xsl:choose>
        <!-- run once -->
        <xsl:when test="not(xalan:nodeset($workentry)/workentry) or xalan:nodeset($workentry)/workentry/trigger/@type = 'simple' and xalan:nodeset($workentry)/workentry/trigger/repeat/@count = 0">
          <xsl:text>once</xsl:text>
        </xsl:when>

        <!-- run infinite -->
        <xsl:when test="xalan:nodeset($workentry)/workentry/trigger/@type = 'simple' and xalan:nodeset($workentry)/workentry/trigger/repeat/@count = -1">
          <xsl:text>infinite</xsl:text>
        </xsl:when>
        
        <!-- run repeatedly -->
        <xsl:when test="xalan:nodeset($workentry)/workentry/trigger/@type = 'simple'">
          <xsl:text>repeatedly</xsl:text>
        </xsl:when>
        
        <!-- run hourly -->
        <xsl:when test="xalan:nodeset($workentry)/workentry/trigger/@type = 'cron' and xalan:nodeset($workentry)/workentry/trigger/hourly">
          <xsl:text>hourly</xsl:text>
        </xsl:when>
        
        <!-- run daily -->
        <xsl:when test="xalan:nodeset($workentry)/workentry/trigger/@type = 'cron' and xalan:nodeset($workentry)/workentry/trigger/daily">
    ...
    <!--xsl:template name="workentry_form_title">
        ...
        <xsl:text> </xsl:text><span id="titlename"><xsl:value-of select="xalan:nodeset($workentry)/workentry/name"/></span>
    </xsl:template-->
    ...
    <xsl:template name="workentryheader">
        ...
        <xsl:text>&nbsp;</xsl:text>
        <span id="titlename">
          <xsl:if test="xalan:nodeset($workentry)/workentry/name != ''">
            <xsl:value-of select="concat(' /', xalan:nodeset($workentry)/workentry/name)"/>
          </xsl:if>
        </span>
    </xsl:template>
        ...
        <div class="tab-pane" id="tab-pane-1">
            ...
            <div class="tab-page" id="tab-page-general">
                    ...
                    <table width="100%" border="0" cellspacing="2" cellpadding="2">
                        <tr>
                            <xsl:call-template name="textfield">
                                ...
                                <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/name"/>
                ...
                <xsl:call-template name="properties">
                  <xsl:with-param name="workentry" select="xalan:nodeset($workentry)/workentry"/>
                </xsl:call-template>
                    ...
                    <table width="100%" border="0" cellspacing="2" cellpadding="2">
                      <tr>
                        <xsl:call-template name="textfielddatetime">
                          ...
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/time/@start"/>
                      ...
                      <tr>
                        <xsl:call-template name="textfielddatetime">
                          ...
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/time/@end"/>
                          <xsl:with-param name="colspan" select="'1'"/>
                          <xsl:with-param name="includeseconds" select="true()"/>
                          <xsl:with-param name="onbluroverridefunction">
                            <xsl:text>dateTimeRangeValidator.validate();</xsl:text>
                      ...
                      <tr id="tr_repeat_interval">
                        ...
                        <xsl:call-template name="textfield">
                          ...
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/repeat/@interval"/>
                          <xsl:with-param name="colspan" select="'1'"/>
                          <xsl:with-param name="required" select="'true'"/>
                      ...
                      <tr id="tr_repeat_count">
                        ...
                        <xsl:call-template name="textfield">
                          ...
                          <xsl:with-param name="size" select="'10'"/>
                          <xsl:with-param name="maxlength" select="'10'"/>
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/repeat/@count"/>
                          <xsl:with-param name="required" select="'true'"/>
                          <xsl:with-param name="colspan" select="'1'"/>
                          <xsl:with-param name="helpelement">
                            <xsl:element name="help">
                      ...
                      <tr id="tr_hourly">
                        ...
                        <xsl:call-template name="textfield">
                          ...
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/hourly/@minutes"/>
                          <xsl:with-param name="colspan" select="'1'"/>
                      ...
                      <tr id="tr_daily">
                        ...
                        <xsl:call-template name="textfield">
                          ...
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/daily/@time"/>
                          <xsl:with-param name="colspan" select="'1'"/>
                      ...
                      <tr id="tr_cron">
                        ...
                        <xsl:call-template name="textfield">
                          ...
                          <xsl:with-param name="selectnode" select="xalan:nodeset($workentry)/workentry/trigger/cron"/>
                          <xsl:with-param name="colspan" select="'1'"/>
                          <xsl:with-param name="size" select="'70'"/>
                          <xsl:with-param name="maxlength" select="'220'"/>
                          <xsl:with-param name="required" select="'true'"/>
    ...
    <xsl:template name="dropdown_types">
        ...
        <td nowrap="nowrap">
            <input type="hidden" name="stepstate_workentry_trigger_@type" id="idtype" value="{xalan:nodeset($workentry)/workentry/trigger/@type}"/>
