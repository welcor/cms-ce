...
<xsl:stylesheet version="1.0" exclude-result-prefixes="#all"
  ...
  <xsl:template match="/">
    <html>
      <head>
        ...
        <script type="text/javascript" language="JavaScript">
          ...
          */
          ...
          if ( fileExtension &amp;&amp; fileExtension === 'png' || fileExtension === 'jpg' || fileExtension === 'jpeg' || fileExtension ===
          'gif' )
          {
          ...
          */
          ...
          opener.addRelatedContent(fieldName, fieldRow, key, title, <xsl:value-of select="$_minoccurrence"/>, <xsl:value-of
            select="$_maxoccurrence"/>);
          }
          }
          }
          window.top.close();
          ...
          */
          ...
          <xsl:if
              test="$subop = 'relatedimage' or $subop = 'relatedfile' or $subop = 'contentfield' or $subop = 'insert' or $only-one-content-is-allowed">
          ...
          */
          ...
          // Make sure that the master button is off. (<xsl:value-of select="($subop != 'relatedimage') or ($subop != 'relatedfile')"/>,
          <xsl:value-of select="$subop"/>)
          ...
          */
          ...
          if (op == 'move_category') {
          window.location = "adminpage?op=move&amp;cat="+
          <xsl:value-of select="$cat"/> +"&amp;newparent="+ categoryKey +"&amp;page=200&amp;oldpage="+ <xsl:value-of select="$page"/>;
          return;
          }
          updateContentKeys();
          ...
          */
          ...
          var url = urlForNavigation + '<xsl:text>&amp;index=</xsl:text><xsl:value-of select="$index"/><xsl:text>&amp;count=</xsl:text>' +
          selectedValue;
      ...
      <body class="jquery-ui">
        ...
        <input type="hidden" name="navigation-url-for-for-count-dropdown" id="navigation-url-for-for-count-dropdown"
               value="{$urlForNavigation}"/>
                  ...
                  <xsl:for-each select="/data/contents/content">
                      ...
                      <script type="text/javascript">
                        ...
                        <xsl:choose>
                          <xsl:when test="$subop = 'relatedcontent'">
                            var isAdded = window.top.opener.getRelatedContent('<xsl:value-of select="$fieldname"/>', '<xsl:value-of
                              select="$fieldrow"/>',<xsl:value-of select="@key"/>);
                          </xsl:when>
                          <xsl:when test="$subop = 'addcontenttosection' or $subop = 'contentfield'">
                            var isAdded = contentAdded(<xsl:value-of select="@key"/>);
                          </xsl:when>
                          <xsl:otherwise>
                            var isAdded = window.top.opener.getRelatedFile('<xsl:value-of select="$fieldname"/>', '<xsl:value-of
                              select="$fieldrow"/>',<xsl:value-of select="@key"/>);
                          </xsl:otherwise>
                        </xsl:choose>
                        } catch(err) {}
                    ...
                    <tr id="content_row_{@key}">
                      ...
                      <td style="padding: 0px;" align="center" onclick="javascript: setChecked('batch_operation{@key}');">
                        ...
                        <input type="checkbox" style="margin:0;padding:0;border:none;" name="batch_operation" id="batch_operation{@key}"
                               value="{@key}" onclick="javascript: setBatchButtonsEnabled(); setChecked('batch_operation{@key}');"/>
                      ...
                      <td class="{$css-class}" style="{concat('width:', $icon-column-width ,'px')}">
                        ...
                        <xsl:call-template name="display-content-icon">
                          ...
                          <xsl:with-param name="contenthandler-class-name"
                                          select="/data/contenttypes/contenttype[@key = current()/@contenttypekey]/@handler"/>
  ...
  <xsl:template name="batchControls">
    ...
    <xsl:if test="$isPopup">
      ...
      <img src="./images/shim.gif" id="batchinsertbtn_shim_{$pos}" title="%alertOneContentOnly%"
